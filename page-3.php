<section class="header color-head">
	<inner-column>
		
		<?php include('modules/header/index.php') ?>

	</inner-column>
</section>

<section class="hero color-hero">
	<inner-column>

		<?php include('modules/hero/index.php'); ?>

	</inner-column>
</section>

<section class="video color-video">
	<inner-column>
		
	<?php include('modules/video/index.php')?>

	</inner-column>
</section>

<section class="grid-list color-grid-list">
	<inner-column>
		
	<?php include('modules/grid-list/index.php')?>
	
	</inner-column>
</section>

<section class="clients color-clients">
	<inner-column>
<?php include('modules/client/template.php')?>
	
	</inner-column>
</section>

<section class="questions color-questions">
	<inner-column>
<?php include('modules/questions/template.php')?>
	
	</inner-column>
</section>

<section class="traditional-cta color-cta">
	<inner-column>
<?php include('modules/traditional-cta/template.php')?>
	
	</inner-column>
</section>



</main>
<section class="footer color-footer">
	<inner-column>

		<?php include('modules/footer/index.php') ?>
	</inner-column>
</section>